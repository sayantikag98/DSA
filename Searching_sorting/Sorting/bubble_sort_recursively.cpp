...
void bubble_sort_recursive(int* arr, int p, int i, int swaps){
	...
	if(i == p){ // for each pass the comparison has reached its end
		...
		bubble_sort_recursive(arr, p-1, 0, 0);  // for going to the next pass
		return;
	}
	...
	bubble_sort_recursive(arr, p, i+1, swaps); // for going to the next element in a pass
...
int main(){
	...
	bubble_sort_recursive(arr, n-1, 0, 0);
